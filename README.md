...
一个面向网页版抖音个人中心粉丝弹窗的用户脚本，用来批量移除粉丝并减少重复手工操作。脚本支持自动识别并跳过“相互关注”用户；当当前已加载出来的粉丝项里，排除“相互关注”后的目标少于 `15` 个时，会自动触发一次触底加载，并在检测到“暂时没有更多了”后停止继续下滑，继续处理当前可见区域里的可删目标。当前版本同时修复了表情昵称和空白昵称粉丝在列表底部被误判为无可删目标的问题。按当前平台限制，每天上限移除 2500 人。
...
- 自动点击“移除”与“确认移除”
- 自动识别“相互关注”按钮并跳过
- 当当前已加载出来的项目里，排除“相互关注”后的目标少于 `15` 个时，自动触发一次触底加载
- 检测到“暂时没有更多了”后停止下滑，继续处理当前可见区域里的可删目标
- 修复表情昵称和空白昵称粉丝在底部区域被误判为不可删除的问题
...
- 当当前已加载出来的非“相互关注”目标少于 `15` 个时，脚本会自动触发一次触底再继续处理
- 如果页面出现“暂时没有更多了”，脚本会停止继续下滑，优先处理当前可见区域里的可删目标
- 如果底部区域出现表情昵称或空白昵称粉丝，脚本会继续将它们识别为可删目标
- 脚本默认每轮会尽量同时处理当前可见区内最多 `5` 个可删目标
- 按当前平台限制，每天最多移除 `2500` 人
